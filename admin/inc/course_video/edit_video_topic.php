<?php
$topic_id = $_GET['edit_video_topic'];
$topic = get_topic_video($topic_id);
?>
...
<div>
  <div class="flex">
    ...
    <div class="md:w-10/12">
      ...
      <div class="ml-3 mt-2">
        <h3 class="text-3xl font-black text-center">Edit topic</h3>
      </div>
      ...
      <form method="POST" action="inc/controller/course_controller.php" class="ml-3 mt-2 px-8" onsubmit="return validation()" enctype="multipart/form-data">
        ...
        <input type="text" name="course_name" id="course_name" class="form-control mt-2" readonly="readonly" value="<?php echo $topic['course_name'] ?>"> <span id="name_error"></span><br>

        <label class="block text-gray-500 font-bold md:text-left mb-1 md:mb-0 pr-4" for="inline-full-name">
          Topic name
        </label>
        <input type="text" name="topic_name" id="topic_name" class="form-control mt-2" value="<?php echo $topic['topic_name'] ?>">
        ...
        <input type="text" name="video" id="video" class="form-control mt-2" value="<?php echo $topic['video_path'] ?>">
        <span id="editor_error" class="text-danger small"></span><br>

        <button type="submit" name="update_video_topic" class="font-bold text-sm px-8 py-2 bg-indigo-600 text-white ml-auto rounded-full float-right">Submit</button>
      </form>
